...
fn main() {
    let input =
        fs::read_to_string("day23/input.txt").expect("Should have been able to read the file");
...
fn part1(input: &str) {
    ...
    while idx >= 0 && (idx as usize) < instructions.len() {
        ...
        println!(
            "{}, {}, {}",
            registers.get(&'g').unwrap_or(&0),
            registers.get(&'e').unwrap_or(&0),
            registers.get(&'b').unwrap_or(&0)
        );
        let output = perform_step_2(instruction, &mut registers);
...
fn perform_step(instruction: &Vec<&str>, registers: &mut HashMap<char, i64>) -> StepOutput {
    match instruction[0] {
        ...
        "rcv" => {
            ...
            // if registers.get(&to_send) == Some(&0) {
            //     return StepOutput {
            //         idx_offset: 1,
            //         frequency: None,
            //         recover: None,
